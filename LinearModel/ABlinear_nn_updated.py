...
def main(argv, num):
    ...
    wd = 0.00087688059438052 #wandb.config.wd
    droprate = 0.0304091024895331954 #wandb.config.dropout
    nn_width = 256 #wandb.config.nn_width
    ...
    # data.drop(data[(data['occ_f2'] > 0) & (data['occ_f2'] < 14)].index, inplace=True)    # f transition 
    ...
    train_database = database[:int(len(database) * tvsplit)]
    val_database = database[int(len(database) * tvsplit):]
    model = SimpleNetwork(len(database[0][0]), droprate, nn_width, funnel).to(device)
    #print(model)
    loss_fn = nn.MSELoss()
    ...
    for i in range(len(val_database)):
